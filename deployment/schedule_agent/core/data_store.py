...
class DataStore:
    ...
    # Base64 File Reading (for AgentCore payload)
    def copy_therapist_file_from_bytes(self, payload: dict) -> Path:
        """Decode base64 therapist CSV and store using existing pattern"""
        if not self._temp_dir:
            raise RuntimeError("Session not initialized")
        data = base64.b64decode(payload["therapist_csv"])
        temp_path = self._temp_dir / "raw" / "temp_therapist.csv"
        with open(temp_path, 'wb') as f:
            f.write(data)
        return self.copy_input(str(temp_path), self.therapist_file)
    
    def copy_prescription_file_from_bytes(self, payload: dict) -> Path:
        """Decode base64 prescription CSV and store using existing pattern"""
        if not self._temp_dir:
            raise RuntimeError("Session not initialized")
        data = base64.b64decode(payload["prescription_csv"])
        temp_path = self._temp_dir / "raw" / "temp_prescription.csv"
        with open(temp_path, 'wb') as f:
            f.write(data)
        return self.copy_input(str(temp_path), self.prescription_file)
    
    def copy_shift_file_from_bytes(self, payload: dict) -> Path:
        """Decode base64 shift Excel and store using existing pattern"""
        if not self._temp_dir:
            raise RuntimeError("Session not initialized")
        data = base64.b64decode(payload["shift_excel"])
        temp_path = self._temp_dir / "raw" / "temp_shift.xlsx"
        with open(temp_path, 'wb') as f:
            f.write(data)
        return self.copy_input(str(temp_path), self.shift_file)
    ...
    # Schedule Operations
    def export_schedule(self, schedule: Schedule) -> None:
    ...
    def save_schedule(self, schedule) -> Path:
        """Save current schedule as pickle file for session persistence."""
        if not self._temp_dir:
            raise RuntimeError("Session not initialized")
        ...
        schedule_file = processed_dir / "current_schedule.pkl"
    ...
    def load_schedule(self):
        """Load current schedule from pickle file."""
        if not self._temp_dir:
            raise RuntimeError("Session not initialized")
        
        schedule_file = self._temp_dir / "processed" / "current_schedule.pkl"
